...
1. **Orient before answering.** Before answering any knowledge question, orient via
   the `/wiki:query` ladder. Never answer bare from parametric knowledge.
2. **Cite or stay silent.** Every wiki-grounded answer must carry a **citation** — a
   page link or a source anchor. No citation, no claim.
...
5. **Capture durable knowledge.** When durable knowledge surfaces mid-conversation,
   do not interrupt the current task — proactively **offer to capture** it.
...
| Import / ingest / "put X in the wiki" / files in inbox | `/wiki:ingest` |
