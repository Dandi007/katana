...
**新规则：`credibility` 声明的是【本条 finding 的支撑引文可核验到什么程度】，不是信源类型。**

> **⚠️ 措辞更正（2026-08-04，接线时发现原表述不可填）**：
> 我最初写的是「这条**引文**的可核验程度」。**填不了** ——
> `credibility` 是 `research.finding.v1` 的字段，而 `anchor`/`quote` 在
> `research.excerpt.v1` 上，**两者是不同的消息**。
> 一条 finding 的 credibility 必须描述**它那几条支撑 excerpt 的整体可核验程度**。

| 值 | 含义（发布前你必须能对自己回答「是」） |
|---|---|
| `high` | 本条的**每一条**支撑 excerpt，引文都是从锚点所指行段**逐字复制**且我重读确认过 |
| `medium` | 引文都出自所指文件，但**行号可能不精确**（凭印象写的行号 / 读的是搜索片段） |
| `low` | 有支撑引文是**概括转述**，或来源不确定 |

**不要因为「这是源码」就填 `high`** —— 源码身份已经写在 `source` 里了，
再写一遍不增加任何信息。

> **为什么这条值得认真填**：`anchor-check` 现在会 **join finding→excerpt**，
> 把你声称的 `credibility` 与**全部支撑引文的实测结果**摆在一起：
> ```
> 声称 high × 全部验得过 : 15
> 声称 high × 部分       : 11
> 声称 high × 一条都验不过 : 1   ← 直接点名该 finding
> ```
> ⇒ **这是一个会被当场对上的断言，不是一个标签。**
> （该 join 是 2026-08-04 补的 —— 此前工具**根本不读 credibility**，
> 我却已经宣称它「可被机械证伪」。**宣称一个检查存在，和它存在，是两件事。**）
