...
const { skillDir } = A
const SOURCES = (A.sources && typeof A.sources === 'object' && !Array.isArray(A.sources)) ? A.sources : {}  // 命名源 { name: entry }；entry=/retrieval:<name>
...
// Logical wiki path. The MCP server owns its physical storage root.
const _dirName = topic.replace(/[/\\:*?"<>|]/g, '-').replace(/\s+/g, ' ').trim().slice(0, 80)
const requestedTopicPath = (typeof A.topicPath === 'string' && A.topicPath.trim())
  ? A.topicPath.trim().replace(/^\/+|\/+$/g, '')
  : ''
const topicPath = requestedTopicPath.startsWith('DeepThought/') && !requestedTopicPath.split('/').includes('..')
  ? requestedTopicPath
  : `DeepThought/${_dirName}`
...
- 命名源（KB config 声明的平台源）：${names.map(n => `${n} → ${SOURCES[n]}`).join(' ； ')}
  ...
  · entry 是 /retrieval:<source> 入口；调用对应 retrieval skill，不把 entry 当文件路径或 CLI。
...
MUST DO:
- 外部源（web/平台）优先经 /retrieval:<source>（retrieval plugin 已装时），无则 fallback 到 WebSearch/WebFetch / 平台 CLI。本地知识只用 /retrieval:search-note 或 wiki_search；据候选路径取正文只用 wiki MCP fs_read。${sourceHints()}
- 按线索的 suggested_sources，每个源单独探索并写一个 per-source 文件：
  · 用 wiki MCP fs_create 写逻辑路径 "${topicPath}/findings/r${round}-c${clue.id}__<源名>.md"（双下划线 __ 分隔源名；创建文件时 server 自动建立父级）
  · 每文件严格按 "${TPL}/finding.md" 模板，frontmatter 填 source/anchor/evidence_credibility/digest/entities；
    body 是该源的 L1 表 + L2 逐字摘录（只摘与线索相关段落，每段必带 anchor）。同时满足 WIKI.md 的 frontmatter/provenance/outlink 不变量。
...
MUST NOT:
- 不写结论、不跨源综合；不修改 findings/ 以外任何 wiki 文件；不对外部源执行 mutation（不发消息/不回复群聊/不开或评论 issue·MR/不 push——平台源一律只读）。`
}
...
并：把 clue_board 快照写到 "${topicPath}/clue_board.md"，按 "${TPL}/clue_board.md" 模板（Frontier=本次 selected，Visited 追加本轮已探线索，每轮摘要追加一行含停止判断）。先用 wiki MCP fs_read 取现有页；不存在时 fs_create，存在时保留 server id 后 fs_write。

返回 TRIAGE_SCHEMA。`
}

function harvesterPrompt() {
  return `TASK: 汇编索引 — 通过 wiki MCP 汇集所有 per-source finding 文件，提取 frontmatter，生成索引表写入 index.md。

1. 用 wiki MCP fs_glob 列出 "${topicPath}/findings/*.md"，再用 fs_read 读取每个文件的 frontmatter（--- 到 --- 之间）。
2. 提取每个文件的：clue_id / round / source / anchor / evidence_credibility / digest / 文件路径。
3. 把结果汇编为符合 WIKI.md 页面不变量的 Markdown 索引（frontmatter 含 provenance/摘要，正文含 outlink），写到 "${topicPath}/findings/index.md"；不存在时用 wiki MCP fs_create，存在时先 fs_read 并保留 server id 后 fs_write：
   格式示例：
   \`\`\`
...
function synthesisPrompt() {
  return `探索已收敛。研究主问题："${topic}"。wiki 素材逻辑路径："${topicPath}"。

按顺序生成最终产物（全部通过 wiki MCP 写入 ${topicPath}/）：
1) 先用 fs_read 读取 "${topicPath}/findings/index.md"（harvest 阶段已汇编的索引表），按 credibility/relevance 选择高价值源。
2) 根据 index.md 索引，用 fs_read 选择性读取高 credibility 或与主问题最相关的 findings/*.md L2 原文（先高后低，跳过 low+不相关的，节省 context）。
...
每个最终 artifact 都必须满足 WIKI.md 的 frontmatter/provenance/outlink 不变量。写入时先 fs_read；不存在用 fs_create，存在则保留 server id 后 fs_write。

MUST: 每个论断可回溯到 sources.md / L2 原文。MUST NOT: 编造来源或修改任何外部信息源。

返回一段 Executive Summary + 3-5 条 Key Takeaways（给对话收尾阶段展示用）。`
}

// Setup phase: validate the logical wiki path and generate initial clues when Stage A was bypassed.
const SETUP_SCHEMA = {
...
if (_needsClues) log(`Stage A not provided — setup agent will validate the wiki path and split clues`)
phase('Setup')

const _setup = await agent(
  `1. 使用 wiki 逻辑路径 "${topicPath}"；不得解析或探测物理根，也不要创建空目录（首个 fs_create artifact 会建立父级）。\n` +
  (_needsClues
  ...
  { phase: 'Setup', schema: SETUP_SCHEMA, label: _needsClues ? 'setup:mcp+clues' : 'setup:mcp' }
)
...
while (frontier.length) {             // 唯一"自然"停止：没线索可探了
  ...
  const found = (await parallel(
    frontier.slice(0, MAX_WIDTH).map(clue => () =>
      agent(workerPrompt(clue, round), {
        ...
        agentType: 'general-purpose',   // worker 检索外部源并通过 wiki MCP 写 L2
  ...
  // triage agent 已在其任务内通过 wiki MCP 重写 clue_board.md 快照
...
// Harvest phase: use wiki MCP to aggregate findings frontmatter into index.md.
phase('Harvest')
await agent(harvesterPrompt(), {
  phase: 'Harvest',
  model: HARVEST_MODEL,
  agentType: 'general-purpose',   // needs wiki MCP fs_glob/fs_read/fs_write
  label: 'harvest:index',
})
