...
2. **worker agentType 不能用 `Explore`**：worker 要通过 wiki MCP `fs_create` 写 L2 findings 文件 → 统一 `general-purpose`（已修，勿回退）。

3. **弱模型 L2 落盘纪律不稳**：L1（schema 返回）可靠，但部分轮次 worker 不写 findings 文件（弱模型实测示例：qwen3.7-max，89 条 L1 findings 只落了 11 个 L2 文件，r3-5/r7 缺失）。综合 agent 只能用已落盘的素材。强模型预计更稳；弱模型跑时可接受或在 worker prompt 加"写完用 wiki MCP `fs_read` 验证文件存在"硬化。
...
8. **Workflow 层撞名**：即使 Skill 路由正确，阶段 B 若用 `Workflow({name: "deep-research"})` 仍会被环境里同名 named workflow 劫持——阶段 A 正常建立 wiki 逻辑路径、报告也会生成，但 clue_board / findings L2 / sources / topics 全缺，平台源证据丢失（2026-06-04 实测）。必须 `scriptPath` 指向本 skill base directory 的 workflow.js（SKILL.md 已加 MUST 硬约束）。
