---
name: deep-research
description: 大规模多源探索并生成研究综述。通过 katana-wiki-mcp 检索本地知识并将产物写入 DeepThought，结合 web 与 config 声明的命名源；不用于单概念快速解释。
---
...
研究产物位于 wiki 逻辑路径 `DeepThought/<主题名>/`，只通过
katana-wiki-mcp 的 `fs_create` / `fs_write` / `fs_read` / `fs_glob` 访问。
client 不解析或接收 wiki 的物理根路径。
...
| `harvest` | `haiku` | 用 wiki MCP 汇编 findings frontmatter → index.md | 纯 IO，haiku 够用且省 token |
...
3. 令 `topicPath="DeepThought/<主题名>"`。用 wiki MCP `fs_read` 读取
   `WIKI.md` 以及 `CLAUDE.md` / `AGENTS.md`（存在时），了解库约定和页面
   frontmatter/provenance/outlink 不变量。不要预建目录；对首个 artifact 调
   `fs_create` 时 server 会建立父级。后续覆盖先 `fs_read`，再用保留 server
   `id` 的 `fs_write`。同时按 env → client-local `.katana` →
   默认读取 `deep_research_sources` / `deep_research_max_width` /
   `deep_research_models`，解析出命名源、宽度与模型映射。
   **worker 档按 topic 意图定**：纯事实聚合类下调 `worker:haiku`；需要读
   代码、推理因果、辨析冲突证据的硬研究上调 `worker:opus`；拿不准用
...
Workflow({
  ...
  args: { topic: "<原问题>", topicPath: "DeepThought/<主题名>",
          ...
          initialClues: [ ...上面拆的线索 ] }
  // topicPath 是 wiki MCP 逻辑路径，不是 client filesystem path
})
```
...
- **harvester 节点**：Explore 后、Synthesize 前，用 wiki MCP `fs_glob` + `fs_read` 汇集全部 `findings/*.md` frontmatter，再用 `fs_write` 写 `findings/index.md`（含 clue_id/round/source/credibility/digest/path 索引表）。
- **synth 索引驱动**：先用 wiki MCP `fs_read` 读取 `index.md`，按 credibility/relevance 选择性读取 L2 正文，不再全量盲读。

## 停止语义
停 = triage `converged=true`（主问题已可充分回答）或 frontier 枯竭。**绝不因轮数/预算/成本停**；`SAFETY_CAP` 仅防脚本失控。

## 恢复未完成研究
用 wiki MCP `fs_glob` / `fs_read` 检查 `DeepThought/<主题>/`：有 `clue_board.md` 快照→从 Frontier 重建 initialClues 再发起 Workflow；有 topics 无 report→只发起 synthesis。有 `findings/index.md` → harvester 已完成，可跳 Harvest 直接 Synthesize。

## 通用规则
- 探索源只读，禁 mutation；本地知识检索优先遵循 KB 约定，兜底必须用 `/retrieval:search-note` 或 wiki MCP `wiki_search`，候选正文只用 wiki MCP `fs_read`。
