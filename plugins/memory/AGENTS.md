...
- **多 runtime 注入**：服务另提供 `GET /t/<tenant>/index.md`（纯文本 `<memory-index>`），供非 Claude runtime 消费；kimi-code（UserPromptSubmit hook + per-session marker）与 OpenCode（plugin `experimental.chat.system.transform` + session 缓存）的客户端在 `runtimes/`，安装走 `runtimes/install.sh`
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `KATANA_MEMORY_MCP_URL` | `http://127.0.0.1:5605` | MCP 服务地址（hook curl 目标） |
...
```bash
# hook 降级路径回归（服务不可达时 fallback JSON 验证）
