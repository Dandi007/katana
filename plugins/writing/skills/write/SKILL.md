...
- **per-type pattern（写的脸）**：若当前项目 writing_dir 下的 patterns/<type>.md 存在，先读它的「写的脸」（适用判定 / 模板骨架 / L0 适配 / 入口 skill 指向）搭骨架。该 pattern 是写、审共享 SSoT——同一文件的「审的脸」由 `writing:readability-check` 消费，review 反馈经 `/readability-check evolve` 固化回此处，写审同步收敛。已固化：`atomic-note` / `tech-spec` / `work-brief`
